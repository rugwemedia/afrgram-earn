import { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import {
    ChevronLeft, Info, Camera, Send,
    CheckCircle2, AlertTriangle, Copy,
    ExternalLink, UserCheck, Loader2, Clock, XCircle
} from 'lucide-react';
import { cn } from '../utils/cn';
import { MediaUploader } from '../components/MediaUploader';
import { useTasks, type Task } from '../context/TaskContext';
import { useAuth } from '../context/AuthContext';

interface TaskDetailProps {
    task: Task;
    onBack: () => void;
}

export function TaskDetail({ task, onBack }: TaskDetailProps) {
    const { user } = useAuth();
    const { submitProof, submissions } = useTasks();
    const [step, setStep] = useState(1);
    const [loading, setLoading] = useState(false);
    const [isCompleted, setIsCompleted] = useState(false);
    const [proofUrl, setProofUrl] = useState('');

    // Find existing submission for this task by this user
    const existingSubmission = submissions.find(
        s => s.taskId === task.id && s.userId === user?.id
    );

    const handleNext = () => setStep(prev => prev + 1);

    const handleSubmit = async () => {
        if (!user || !proofUrl) return;

        setLoading(true);
        try {
            // Simulate network delay
            await new Promise(resolve => setTimeout(resolve, 1500));

            submitProof({
                taskId: task.id,
                userId: user.id,
                userEmail: user.email!,
                userName: user.user_metadata?.full_name || 'User',
                proofImage: proofUrl,
            });

            setIsCompleted(true);
        } catch (error) {
            console.error(error);
        } finally {
            setLoading(false);
        }
    };

    // --- Blocked: pending submission ---
    if (existingSubmission && existingSubmission.status === 'pending') {
        return (
            <div className="max-w-2xl mx-auto pb-20">
                <button
                    onClick={onBack}
                    className="flex items-center gap-2 text-muted-foreground hover:text-white transition-colors mb-8 font-bold"
                >
                    <ChevronLeft size={20} /> Back to Dashboard
                </button>
                <div className="glass-card p-10 rounded-[2.5rem] flex flex-col items-center text-center space-y-6">
                    <div className="w-20 h-20 bg-amber-500/10 border border-amber-500/25 rounded-full flex items-center justify-center shadow-[0_0_40px_rgba(245,158,11,0.25)]">
                        <Clock size={40} className="text-amber-500" />
                    </div>
                    <div className="space-y-2">
                        <h2 className="text-2xl font-black text-white">Awaiting Admin Review</h2>
                        <p className="text-muted-foreground max-w-sm mx-auto text-sm leading-relaxed">
                            You have already submitted proof for <b className="text-white">{task.title}</b>.
                            Your submission is currently being reviewed by an admin.
                        </p>
                        <p className="text-amber-500 font-bold text-sm">
                            You can only re-submit if your proof is rejected.
                        </p>
                    </div>
                    <div className="w-full bg-white/5 rounded-2xl p-4 border border-white/10 text-left">
                        <p className="text-xs font-black text-muted-foreground uppercase tracking-widest mb-1">Submitted</p>
                        <p className="text-sm text-white font-bold">
                            {new Date(existingSubmission.submittedAt).toLocaleString()}
                        </p>
                    </div>
                    <button onClick={onBack} className="btn-premium px-10">Back to Tasks</button>
                </div>
            </div>
        );
    }

    // --- Blocked: approved submission ---
    if (existingSubmission && existingSubmission.status === 'approved') {
        return (
            <div className="max-w-2xl mx-auto pb-20">
                <button
                    onClick={onBack}
                    className="flex items-center gap-2 text-muted-foreground hover:text-white transition-colors mb-8 font-bold"
                >
                    <ChevronLeft size={20} /> Back to Dashboard
                </button>
                <div className="glass-card p-10 rounded-[2.5rem] flex flex-col items-center text-center space-y-6">
                    <div className="w-20 h-20 bg-emerald-500/10 border border-emerald-500/25 rounded-full flex items-center justify-center shadow-[0_0_40px_rgba(16,185,129,0.25)]">
                        <CheckCircle2 size={40} className="text-emerald-500" />
                    </div>
                    <div className="space-y-2">
                        <h2 className="text-2xl font-black text-white">Task Completed!</h2>
                        <p className="text-muted-foreground max-w-sm mx-auto text-sm leading-relaxed">
                            Your submission for <b className="text-white">{task.title}</b> has been verified and approved.
                        </p>
                        <p className="text-emerald-500 font-bold text-lg">+{task.reward} RWF Added</p>
                    </div>
                    <button onClick={onBack} className="btn-premium px-10">Back to Tasks</button>
                </div>
            </div>
        );
    }

    // --- Rejected: allow re-submission ---
    if (existingSubmission && existingSubmission.status === 'rejected') {
        // Show banner but allow continuing - fall through to normal flow
    }

    // --- Already submitted just now ---
    if (isCompleted) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-6">
                <div className="w-24 h-24 bg-emerald-500/10 border border-emerald-500/20 rounded-full flex items-center justify-center shadow-[0_0_50px_rgba(16,185,129,0.3)]">
                    <UserCheck size={48} className="text-emerald-500" />
                </div>
                <div className="space-y-2">
                    <h2 className="text-3xl font-black text-white">Proof Submitted!</h2>
                    <p className="text-muted-foreground max-w-sm mx-auto">
                        Our verification engine will review your submission within 1-2 hours.
                        <b> {task.reward} RWF</b> will be added to your balance pending approval.
                    </p>
                </div>
                <button onClick={onBack} className="btn-premium px-10">Back to Dashboard</button>
            </div>
        );
    }

    return (
        <div className="max-w-2xl mx-auto pb-20">
            <button
                onClick={onBack}
                className="flex items-center gap-2 text-muted-foreground hover:text-white transition-colors mb-8 font-bold"
            >
                <ChevronLeft size={20} /> Back to Dashboard
            </button>

            <header className="flex justify-between items-start mb-10">
                <div>
                    <h1 className="text-3xl font-black text-white tracking-tight">{task.title}</h1>
                    <p className="text-muted-foreground mt-1 flex items-center gap-2">
                        {task.description}
                    </p>
                </div>
                <div className="bg-emerald-500 text-black font-black px-4 py-2 rounded-xl shadow-[0_0_15px_rgba(16,185,129,0.4)]">
                    +{task.reward} RWF
                </div>
            </header>

            {/* Rejected re-submission warning */}
            {existingSubmission?.status === 'rejected' && (
                <div className="mb-6 bg-red-500/10 border border-red-500/20 p-4 rounded-2xl flex items-start gap-3">
                    <XCircle size={18} className="text-red-500 shrink-0 mt-0.5" />
                    <div>
                        <p className="text-sm text-red-400 font-bold">Previous submission was rejected</p>
                        <p className="text-xs text-red-400/70 mt-0.5">You may submit new proof below.</p>
                    </div>
                </div>
            )}

            {/* Stepper */}
            <div className="flex justify-between items-center mb-12 relative">
                <div className="absolute h-1 bg-white/5 left-0 right-0 top-1/2 -translate-y-1/2 -z-10" />
                {[1, 2, 3].map(s => (
                    <div
                        key={s}
                        className={cn(
                            "w-10 h-10 rounded-full flex items-center justify-center font-black transition-all duration-500",
                            s < step ? "bg-primary text-white scale-90" :
                                s === step ? "bg-primary text-white shadow-[0_0_20px_rgba(59,130,246,0.6)]" :
                                    "bg-accent text-muted-foreground border border-white/10"
                        )}
                    >
                        {s < step ? <CheckCircle2 size={20} /> : s}
                    </div>
                ))}
            </div>

            <AnimatePresence mode="wait">
                <motion.div
                    key={step}
                    initial={{ opacity: 0, x: 20 }}
                    animate={{ opacity: 1, x: 0 }}
                    exit={{ opacity: 0, x: -20 }}
                    className="glass-card p-10 rounded-[2.5rem] space-y-8"
                >
                    {step === 1 && (
                        <div className="space-y-6">
                            <h3 className="text-xl font-bold text-white flex items-center gap-3">
                                <Info className="text-primary" /> Step 1: Instructions
                            </h3>
                            <p className="text-muted-foreground text-sm leading-relaxed whitespace-pre-line">
                                {task.instruction}
                            </p>

                            {task.type === 'gmail' && (
                                <div className="space-y-3 bg-white/[0.02] p-6 rounded-2xl border border-white/5">
                                    {[
                                        { label: 'Email Format', value: task.requiredEmailFormat },
                                        { label: 'Password', value: task.requiredPassword },
                                        { label: 'Recovery Email', value: task.recoveryEmail },
                                    ].map((item, i) => (
                                        item.value && (
                                            <div key={i} className="flex justify-between items-center group">
                                                <span className="text-xs font-black text-muted-foreground uppercase">{item.label}</span>
                                                <div className="flex items-center gap-3">
                                                    <code className="text-sm font-bold text-white font-mono">{item.value}</code>
                                                    <button className="opacity-0 group-hover:opacity-100 text-primary transition-opacity"><Copy size={14} /></button>
                                                </div>
                                            </div>
                                        )
                                    ))}
                                </div>
                            )}

                            <button onClick={handleNext} className="btn-premium w-full flex items-center justify-center gap-2">
                                I Understand <ExternalLink size={18} />
                            </button>
                        </div>
                    )}

                    {step === 2 && (
                        <div className="space-y-6">
                            <h3 className="text-xl font-bold text-white flex items-center gap-3">
                                <Camera className="text-primary" /> Step 2: Proof Capture
                            </h3>
                            <p className="text-muted-foreground text-sm">
                                Please upload a screenshot verifying you have completed the task.
                            </p>

                            <div className="aspect-video bg-white/5 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center gap-4 group hover:border-primary/50 transition-all cursor-pointer relative overflow-hidden">
                                <MediaUploader
                                    label="Upload Screenshot Proof"
                                    className="w-full h-full border-none hover:bg-transparent"
                                    onUploadComplete={(url) => {
                                        setProofUrl(url);
                                        handleNext();
                                    }}
                                />
                            </div>

                            <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-xl flex items-start gap-3">
                                <AlertTriangle size={18} className="text-red-500 shrink-0 mt-0.5" />
                                <p className="text-xs text-red-500 leading-relaxed font-bold">
                                    Warning: Submitting fake proof will result in account suspension.
                                </p>
                            </div>
                        </div>
                    )}

                    {step === 3 && (
                        <div className="space-y-6">
                            <h3 className="text-xl font-bold text-white flex items-center gap-3">
                                <Send className="text-primary" /> Step 3: Final Submission
                            </h3>
                            <p className="text-muted-foreground text-sm">
                                Confirm your submission.
                            </p>

                            {proofUrl && (
                                <div className="rounded-xl overflow-hidden border border-white/10 h-48 bg-black/50">
                                    <img src={proofUrl} alt="Proof Preview" className="w-full h-full object-contain" />
                                </div>
                            )}

                            <button
                                onClick={handleSubmit}
                                disabled={loading}
                                className="btn-premium w-full flex items-center justify-center gap-2"
                            >
                                {loading ? (
                                    <Loader2 size={20} className="animate-spin" />
                                ) : (
                                    <>Submit Proof For Review <Send size={18} /></>
                                )}
                            </button>
                        </div>
                    )}
                </motion.div>
            </AnimatePresence>
        </div >
    );
}
